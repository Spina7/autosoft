...
function page_require_level($require_level)
{
  ...
  elseif ($login_level['group_status'] ?? 'default value' == '0' ?? 'default value') :
...
/* Función para encontrar todos los nombres de productos
   ...
   /*--------------------------------------------------------------*/
function loads_table()
{
  global $db;
  $sql  = " SELECT CONCAT('OLS0',l.id) AS id,l.PO,t.pickup,t.dispatched,q.name AS client, CONCAT(a.city,', ',a.state) AS origin, CONCAT(a2.city,', ',a2.state) AS destination, b.name AS broker,c1.name";
  $sql  .= " AS carrier_usa,c2.name AS carrier_mx,l.trailer_usa,l.trailer_mx,l.truck,l.driver_name, l.status";
  $sql  .= " FROM loads l LEFT JOIN timelines t ON t.id =l.id LEFT JOIN clients q ON q.id = l.client_id";
  $sql  .= " LEFT JOIN carriers c1 ON c1.id = l.carrier_usa_id LEFT JOIN carriers c2 ON c2.id = l.carrier_mx_id";
  $sql  .= " LEFT JOIN  warehouse_addresses a ON a.id = l.origin_address_id LEFT JOIN  warehouse_addresses a2 ON a2.id = l.destination_address_id";
  $sql  .= " LEFT JOIN custom_brokers b ON b.id = l.custom_broker_id ORDER BY l.id DESC";
  return find_by_sql($sql);
}

function southbound_total_count()
{
  $sql  = "SELECT COUNT(*) AS total FROM loads WHERE type = 'IMPORTACION';";

  return find_by_sql($sql);
}

function south_toPickup_count()
{
  $sql  = "SELECT COUNT(*) AS total FROM loads WHERE type = 'IMPORTACION' AND status = 'PROGRAMADA';";

  return find_by_sql($sql);
}

function north_toPickup_count()
{
  $sql  = "SELECT COUNT(*) AS total FROM loads WHERE type = 'EXPORTACION' AND status = 'PROGRAMADA';";

  return find_by_sql($sql);
}

function northbound_total_count()
{
  $sql  = "SELECT COUNT(*) AS total FROM loads WHERE TYPE = 'EXPORTACION';";
